 ...
 *
 * `de_mirage`: в репозитории остаются только точки, для которых есть контент;
 * прочие placeholder-callouts (Рампа, Палас, …) не храним — добавляйте позиции
 * через `position-catalog-extensions.json` или админку каталога.
 *
